...
(function() {
	...
	var pageKey = /[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]{2,4}$/i.test(window.location.hostname) ? 'banned_'+ window.location.hostname.match(/[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]{2,4}$/i)[0] : 'empty',
		version = 3,
		hovering = null,
		firstKey = '',
		toggle = false,
	...
	function init()
	{
		var defaultSetup = {version: version, ids: {}, classes: {}, actions: {}, monitorMutations: false};
		try {
	...
	function applyData()
	{
		...
		for (var className in bannedData.classes) {
		...
		setTimeout(function () {
			for (var target in bannedData.actions) {
				var action = bannedData.actions[ target ];
	...
	{
		if ((firstKey == 'Shift' && event.key == 'Alt') || (firstKey == 'Alt' && event.key == 'Shift')) {
			toggle = true;
			toggleObserver();
	...
	function focus(event)
	{
		...
		helper.style.left = (event.clientX + 150 > window.innerWidth ? event.clientX - 160 : event.clientX) + 'px';
		helper.innerHTML = '<span style="color:#A886BA">'+event.target.tagName.toLowerCase() +'</span><span style="color:'+
			(event.target.id ? '#FFC66D">#'+event.target.id : '#88B7C5">.'+event.target.className) + '</span>';
	}
	...
	function displayItem(info)
	{
		...
		var items = document.getElementsByClassName(info.val);
		for (var i = 0; i < items.length; ++i) {
	...
	function showAbort(event)
	{
		...
		abort.style.left = (event.clientX + 150 > window.innerWidth ? event.clientX - 160 : event.clientX) + 'px';
	...
	var globalMouseMove = function(event)
	{
	...
	var deleteElem = function(event)
	{
		event.preventDefault();
		blur(event.target);

		if ('body' == document.body.tagName.toLowerCase() &&
			document.getElementsByTagName('div')[0].ownerDocument.defaultView === window.top) {
			return;
		}
		...
		if (canSaveForFurtherPageLoads) {
			...
			for (var i = 0; i < classUsersCount; ++i) {
	...
	var initObserver = function(event)
	{
	...
	var stopObserver = function(event)
	{
	...
	function delayedDeleteByClass(event, classUsers, className)
	{
		var i = 0,
			classUsersCount = classUsers.length,
			canSaveForFurtherPageLoads = confirm('This item can only be identified by its class and has '+ classUsersCount +' siblings (shown in green), do you want to save this setting anyway?');

		for (; i < classUsersCount; ++i) {
			classUsers[i].style.background = '';
		}

		toggleOff({key: 'Alt'}); // simulating a keyup event

		if (canSaveForFurtherPageLoads) {
			for (i = 0; i < classUsersCount; ++i) {
				classUsers[i].style.setProperty('display', 'none', 'important');
				//classUsers[i].style.display = 'none';
			}
	...
	function applySpecialAction(identifier, property, value)
	{
		...
		switch (type)
		{
			...
			case 'css':
				...
				style.appendChild(document.createTextNode(name +' {'+ property +': '+ value +'}'));
				return true;
		}
